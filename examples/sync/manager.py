...
from opensearch_toolkits.core import OpenSearchConfig, OpenSearchManager

load_dotenv(find_dotenv())  # .env 파일에서 환경 변수 로드

# OpenSearchManager 동기 방식 사용 예제
def sync_example():
    """OpenSearchManager의 동기 방식 기능을 시연하는 예제"""
    
    print("=== OpenSearchManager 동기 방식 예제 ===\n")
    ...
    manager = OpenSearchManager(config)
    
    # 2. 연결 확인
    print("## 연결 상태 확인")
    connection_status = manager.check_connection()
    print(f"동기 연결 상태: {'성공' if connection_status else '실패'}")
    print()
    ...
    if manager.index_exists(test_index_name):
        ...
        success = manager.update_index_settings(test_index_name, settings)
        print(f"설정 업데이트 결과: {'성공' if success else '실패'}")
        
        # 6.5 인덱스 복제
        clone_index_name = f"{test_index_name}-clone"
        print(f"## '{test_index_name}'를 '{clone_index_name}'로 복제")
        
        # 대상 인덱스가 이미 있는지 확인
        if manager.index_exists(clone_index_name):
            print(f"'{clone_index_name}' 인덱스가 이미 존재합니다.")
        else:
            success = manager.clone_index(test_index_name, clone_index_name)
